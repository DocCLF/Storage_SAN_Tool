    ...
    process {
        ...
        if((Get-ChildItem -Path $TD_UsedLogPath -File).Count -lt 2){
            ...
            try {
                ...
                Get-ChildItem -Path $TD_UsedLogPath -Attributes $TD_DeleteWhat -Exclude *.xaml,*png,*.dll,*.db -Recurse -File | Where-Object { $_.LastWriteTime -lt $CutoffDate} | Remove-Item -Confirm:$false -Force -ErrorAction SilentlyContinue

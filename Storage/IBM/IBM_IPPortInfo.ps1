function IBM_IPPortInfo {
    ...
    begin {
        ...
        if($TD_Device_ConnectionTyp -eq "ssh"){
            $TD_CollectIPPortInfos = ssh -i $($TD_Device_SSHKeyPath) $TD_Device_UserName@$TD_Device_DeviceIP 'lsip -nohdr -delim :'
        }else {
    ...
    process {
        ...
        if(($TD_Line_ID -eq 1)-and($TD_IPPortInfoResault.count -lt 1)){$TD_LB_IPPortInfoOne.Visibility = "Visible";  $TD_LB_IPPortInfoOne.Content = "There is NO IP Port Info for $($TD_Device_DeviceName)."}
        if(($TD_Line_ID -eq 2)-and($TD_IPPortInfoResault.count -lt 1)){$TD_LB_IPPortInfoTwo.Visibility = "Visible";  $TD_LB_IPPortInfoTwo.Content = "There is NO IP Port Info for $($TD_Device_DeviceName)."}
        if(($TD_Line_ID -eq 3)-and($TD_IPPortInfoResault.count -lt 1)){$TD_LB_IPPortInfoThree.Visibility = "Visible";$TD_LB_IPPortInfoThree.Content = "There is NO IP Port Info for $($TD_Device_DeviceName)."}
        if(($TD_Line_ID -eq 4)-and($TD_IPPortInfoResault.count -lt 1)){$TD_LB_IPPortInfoFour.Visibility = "Visible"; $TD_LB_IPPortInfoFour.Content = "There is NO IP Port Info for $($TD_Device_DeviceName)."}
        if(($TD_Line_ID -eq 5)-and($TD_IPPortInfoResault.count -lt 1)){$TD_LB_IPPortInfoFive.Visibility = "Visible"; $TD_LB_IPPortInfoFive.Content = "There is NO IP Port Info for $($TD_Device_DeviceName)."}
        if(($TD_Line_ID -eq 6)-and($TD_IPPortInfoResault.count -lt 1)){$TD_LB_IPPortInfoSix.Visibility = "Visible";  $TD_LB_IPPortInfoSix.Content = "There is NO IP Port Info for $($TD_Device_DeviceName)."}
        if(($TD_Line_ID -eq 7)-and($TD_IPPortInfoResault.count -lt 1)){$TD_LB_IPPortInfoSeven.Visibility = "Visible";$TD_LB_IPPortInfoSeven.Content = "There is NO IP Port Info for $($TD_Device_DeviceName)."}
        if(($TD_Line_ID -eq 8)-and($TD_IPPortInfoResault.count -lt 1)){$TD_LB_IPPortInfoEight.Visibility = "Visible";$TD_LB_IPPortInfoEight.Content = "There is NO IP Port Info for $($TD_Device_DeviceName)."}

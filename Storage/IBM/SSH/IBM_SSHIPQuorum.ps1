        ...
        switch ($TD_DeviceInformation.Count) {
            {($_ -ge 3)} { 
                $TD_Quorum = foreach($TD_Line in $TD_DeviceInformation) {
                    ...
                    $TD_QuorumInfo.SiteName = ($TD_Line|Select-String -Pattern '(yes|no|):(drive|device):(yes|no):(|\d+):(|[\w\-\.\/]+)$'-AllMatches).Matches.Groups[5].Value <#here is a missmatch with 8.7.x in a PBHA setting#>
